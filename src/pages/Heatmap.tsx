    ...
    return (
        <div className={cn("w-full", className)}>
            <div className="bg-black p-4 md:p-8 rounded-lg">
                <h2 className="text-center text-gray-300 text-base md:text-lg mb-4 md:mb-6">
                    Receptionist call volume heatmap (7 days, 24 hours)
                </h2>
                ...
                <div className="flex flex-row flex-nowrap items-stretch gap-4">
                    ...
                    <div className="flex-1 min-w-0 overflow-x-auto">
                        ...
                        <div className="space-y-0">
                            {data.map((row, rowIndex) => (
                                ...
                                >
                                    {/* Day label */}
                                    <div className="w-16 md:w-20 text-gray-500 text-xs md:text-sm shrink-0">
                                        {row.day}
                                    </div>

                                    {/* Hour cells */}
                                    <div className="flex gap-0 flex-1 min-w-[480px] md:min-w-0">
                                        {row.hours.map((value, colIndex) => (
                                            <div
                                                key={colIndex}
                                                className="h-6 md:h-8 flex-1"
                                                style={{
                                                    backgroundColor:
                                                        getColor(value),
                                                }}
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            ...
                            <div className="flex gap-1 flex-1 min-w-[480px] md:min-w-0">
                                {hours.map((hour) => (
                                    <div
                                        key={hour}
                                        className="flex-1 text-center text-gray-500 text-[10px] md:text-xs"
                                    >
                                        {hour % 2 === 0
                                            ? `0${hour}:00`.slice(-5)
                                            : ""}
                    ...
                    {/* Legend */}
                    <div className="flex flex-col justify-between py-1 md:ml-4 shrink-0 w-10 md:w-auto">
                        <div className="flex flex-col items-end gap-0">
                            {[16, 14, 12, 10, 8, 6, 4, 2, 0].map((value) => (
                                ...
                                >
                                    ...
                                    <div
                                        className="hidden md:block w-4 h-6"
                                        style={{
                        ...
                        {/* Vertical label on md+, horizontal on mobile */}
                        <div className="hidden md:block text-gray-500 text-xs mt-2 -rotate-90 origin-center whitespace-nowrap -translate-y-36 mr-1">
                            Number of calls
                        </div>
                        <div className="block md:hidden text-gray-500 text-xs mt-3 text-center">
                            Numbers
                        </div>
                    </div>
                </div>
